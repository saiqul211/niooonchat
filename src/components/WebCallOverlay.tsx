...
import { Mic, MicOff, Video, VideoOff, PhoneOff, Phone, Volume2, VolumeX, ShieldCheck } from 'lucide-react';
...
export const WebCallOverlay: React.FC = () => {
  ...
  const { type, status, isIncoming, participant, durationSeconds, isMicMuted, isVideoOff } = callState;
    ...
    >
      ...
      <div className="w-full max-w-md flex items-center justify-between pt-4">
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-neutral-900/90 border border-neutral-800 text-xs font-semibold tracking-wider text-neutral-400">
          ...
          <span>{type === 'video' ? 'END-TO-END ENCRYPTED VIDEO' : 'END-TO-END ENCRYPTED AUDIO'}</span>
        </div>
      </div>
      ...
      {type === 'video' && status === 'connected' && (
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
          {/* Remote Video Placeholder or Stream */}
          ...
          {/* Local Video PIP */}
          {!isVideoOff && (
            <div className="absolute top-6 right-6 w-32 h-44 sm:w-40 sm:h-56 rounded-2xl overflow-hidden bg-neutral-900 border-2 border-neutral-800 shadow-2xl z-10 pointer-events-auto">
              <video
                ref={localVideoRef}
                autoPlay
                playsInline
      ...
      <div className="relative z-10 flex flex-col items-center justify-center my-auto text-center">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-neutral-900/80 border border-neutral-800 text-sm font-medium">
          {status === 'outgoing' && <span className="text-blue-400">Calling...</span>}
          {status === 'ringing' && <span className="text-amber-400">Incoming call...</span>}
          {status === 'connecting' && <span className="text-emerald-400">Connecting...</span>}
          {status === 'connected' && (
      ...
      {/* Bottom Controls Bar */}
      <div className="relative z-10 w-full max-w-md pb-6 flex flex-col items-center gap-6">
